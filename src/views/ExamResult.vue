<template>
    <div>
        ...
        <div style="text-align: center;margin-bottom:5px;font-size: 15px;">{{ExamObj.createTime}}</div>
        <div class="box1"><span>考生姓名:{{ username }}</span><span>考试用时:{{ ExamObj.userTime }}min</span><span>考试得分:{{ ExamObj.userScore }}</span></div>
        <div class="ExamList" v-for="item,index in ExamObj.quList">
            <div style="padding: 5px; background-color: #fff; border-radius: 5px;width: 343px;margin: 0 auto;">{{index+1}}.{{ item.content }}</div>
          ...
          <van-cell-group inset>
            ...
            >
              <template #right-icon>
                <van-checkbox :name="item1.id" :ref="(el) => (checkboxRefs[index1] = el)" @click.stop />
              </template>
            </van-cell>
          </van-cell-group>
          <div class="tishi"><span style="color: green;">正确答案:{{ item.RigitZimu }}</span> <span :style="item.isRight?'color:green':'color:red'">答案结果:{{ item.clickZimu }}</span></div>
        </van-checkbox-group>
        </div>
    </div>
</template>
